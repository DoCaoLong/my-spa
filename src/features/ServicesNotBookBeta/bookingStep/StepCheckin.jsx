...
function StepCheckin(props) {
      ...
      const { timeBooking, brBooking, serBooking, orgBooking, orderId, setOrgBooking, setSerBooking } = useContext(AppContext);
      ...
      const userInfo = acc;
      // const service_ids = [];
      // console.log(serBooking)
      //  for (var item of serBooking) {
      //        const id = item.sid
      //        service_ids.push(id);
      //  }
      const service_ids = serBooking.map(item => item?.serviceable?.sid)
      useEffect(() => {
            if (brBooking) {
                  setParams({
                        order_id: orderId,
      ...
      const handleSubmit = () => {
            //console.log(orgBooking.id,params)
